...
If a server crashes or become unresponsive.

The load balancer detects it and it reroutes the request to other healthy servers, ensuring the container
...
To put all this in perspective, consider a high traffic e-commerce website like Amazon during a sale

event.

Without load balancing, a sudden influx of users could overwhelm a single server, causing slow performance

or even downtime in some cases.
...
Next, let's explore the different types of load balancers and how they function.
...
They operate on the application layer.

The layer seven load balancers inspect the actual request contents also.
...
Apart from the network layer load balancer are also cache classified based on how they are deployed.

So there are three main categories for this hardware software and cloud based load balancers.

So what are hardware based load balancers hardware load balancers.
...
Software load balancers are the applications that run on general purpose servers.

They provide flexibility and cost effectiveness.

Because no dedicated hardware is required, they can be deployed on any virtual machine, container
...
Some examples are AWS Elastic Load balancers, Google Cloud load balancers.
...
Choosing the right type of load balancers depend on the use case.

What performance requirements are there and what infrastructure requirements are there?
...
These strategies work very well in environments where the server capacities are similar and the workloads
...
For example, if there are three servers, the first request goes to server one, the second request

goes to server two, the third request goes to server three, and then the cycle repeats on continuously.
...
Now the static strategies are simple and effective in certain scenarios, but they may not be very optimal

for systems with fluctuating Fitting workloads.
...
For instance, if one of the server is expiring high CPU usage, The load balancer will dynamically
...
dynamic strategies like adaptive load balancing are much more effective.

Now that we know different load balancing strategies, let's look at some real world scenarios and how

load balancers operate in action.

So imagine we have a web application that serves thousands or even millions of users daily.

These applications run on multiple back end servers to handle this load efficiently.
...
or making a payment or submitting a form, the load balancer acts as an intermediary instead of requests

going to a single server.
...
So the first decision that we have to think about is whether we want to use a layer four or a layer

seven load balancer.

We use a layer four load balancer when we want fast and efficient traffic distribution, purely based

on network level data like IP addresses and ports.

It is ideal for handling TCP and UDP traffic where the content inspection is not required.
...
The second thing that we need to think about is the scalability needs as our application grows.

Handling the increasing traffic becomes a challenge, so choosing the right load balancer depends on

the expected workload also.
...
But for high traffic applications, cloud based load balancers like AWS or Google Cloud Load Balancers,

they will automatically scale based on demand for enterprise level workloads.

Hardware level load balancers are much better suited as they provide lower latency, and they are ideal
...
The third thing that we need to consider is about security.

SSL termination and DDoS protection is something that every system designer should be aware of.
...
So if we choose a load balancer that provides this capability of SSL termination, offloading this process

from the back end server will actually improve the performance of your overall application by a lot.
...
They detect and filter out malicious traffic before it even reaches the back end servers.
...
So the questions that you are looking on my screen.

These questions test both theoretical knowledge and practical decision making in designing scalable

systems, since whatever we have covered so far all the fundamental concepts, I think you should be

able to answer most of these questions very confidently.
...
Layer four, layer seven load balancers.
...
headers, URL, query strings, and cookies.

We have also looked at different deployment model, hardware based, software based and cloud based

load balancer each serve different scalability and requirement need.
...
Now that we understand the load balancing, the next logical step is to discuss API gateways in modern

architecture, especially with authentication, rate limiting and routing requests efficiently.

They often integrate load balancing with the API gateway functionalities, which are these rate limiting

and all, and they try to optimize the traffic distribution even further.

So in the next lecture, we will dive into the API gateways, their role in the system design and how

they differ from traditional load balancer.

So this wraps up this lecture on load balancing.

If you have any questions, feel free to refer back to the slides or the attached PDF, or reach out

to me in the comment section, so I will see you in the next one.